...
%         d * e1 * B(k-j1) - a1 * B(k-j1))* (h^(p2)) - sum_B;
    ...
    j1 = 1;
    while (k-j1)>0
        sum1_C = sum1_C + (1/(h^p3))*((-1))*(p3/j1)*(Cvec(k-j1));
        j1 = j1+1;
    end
%     C(k) = sum1_C - summation(C,p3,v(k,L,h),k);
    C(k) = sum1_C*(h^p3) - sum_C;
...
%     C(k) = ((1-d)*f1*B(k) - (g1 + a1)*C(k-j1))*(h^p3) - sum_C;
    ...
    j1 = 1;
    while (k-j1)>0
        sum1_D = sum1_D + (1/(h^p4))*((-1))*(p4/j1)*(Dvec(k-j1));
        j1 = j1+1;
    end
%     D(k) = sum1_D - summation(D,p4,v(k,L,h),k);
    D(k) = sum1_D*(h^p4) - sum_D;
...
%     D(k) = (d*e1*B(k) - (g2 + a1)*D(k-j1))*(h^p4) - sum_D;
    ...
    j1 = 1;
    while (k-j1)>0
        sum1_E = sum1_E + (1/(h^p5))*((1)^j1)*(p5/j1)*(Evec(k-j1));
        j1 = j1+1;
    end
...
%     E(k) = (g1*C(k) + g2*D(k) - a1*E(k-j1))*(h^p5) - sum_E;
    ...
    j1 = 1;
    while (k-j1)>0
        sum1_F = sum1_F + (1/(h^p6))*((1)^j1)*(p6/j1)*(Fvec(k-j1));
        j1 = j1+1;
    end
...
function answer = funcA(t)
    ...
    elseif t == 0
        ...
        it = 0;
        while it<t
            answer = answer + (mu1 - a1*funcA(it) - (b1*funcA(it)*(funcC(it) + (sigma*funcD(it))))/N )*dt;
            it = it + dt;
        end
        Avec(floor((t/h)+1)) = answer;
    end
...
function answer = funcC(t)
    ...
    elseif t == 0
        ...
        it = 0;
        while it<t
            answer = answer + ((1-d)*f1*funcB(it) - (g1 + a1)*funcC(it) )*dt;
            it = it + dt;
        end
        Cvec(floor((t/h)+1)) = answer;
    end
...
function answer = funcD(t)
    ...
    elseif t == 0
        ...
        it = 0;
        while it<t
            answer = answer + (d*e1*funcB(it) - (g2 + a1)*funcD(it))*dt;
            it = it + dt;
        end
        Dvec(floor((t/h)+1)) = answer;
    end
...
function answer = funcE(t)
    ...
    elseif t == 0
        ...
        it = 0;
        while it<t
            answer = answer + (g1*funcC(it) + g2*funcD(it) - a1*funcE(it))*dt;
            it = it + dt;
        end
        Evec(floor((t/h)+1)) = answer;
    end
...
function answer = funcF(t)
    ...
    elseif t == 0
        ...
        it = 0;
        while it<t
            answer = answer + (e2*funcC(it) + f2*funcD(it) - mu2*funcF(it))*dt;
            it = it + dt;
        end
        Fvec(floor((t/h)+1)) = answer;
    end
